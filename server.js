...
const ADMIN_PASSWORD = 'jjjiiidddlllee'; // пароль открытой панели хоста (кик / бан / рестарт раунда)

// бафф скорости — только в командном бою, спавнится каждые 30 сек, даёт x2 скорости на 15 сек
const SPEED_BUFF_INTERVAL = 30000;
const SPEED_BUFF_DURATION = 15000;
const SPEED_BUFF_MULT     = 2;
const SPEED_BUFF_RADIUS   = 40;
...
class GameRoom {
  constructor(code, mode) {
    ...
    this.speedBuff = null;
    this.nextBuffSpawnAt = mode === 'team' ? Date.now() + SPEED_BUFF_INTERVAL : Infinity;
  }
}

function spawnSpeedBuff(room) {
  for (let i = 0; i < 40; i++) {
    const x = 150 + Math.random() * (MAP_W - 300);
    const y = 150 + Math.random() * (MAP_H - 300);
    if (!collidesAnyIn(room.obstacles, x, y, SPEED_BUFF_RADIUS)) {
      room.speedBuff = { x, y, r: SPEED_BUFF_RADIUS };
      return;
    }
  }
}
...
wss.on('connection', (ws, req) => {
  ws.isAlive = true;
  ws.isAdmin = false;
  ws.remoteIP = ((req.headers['x-forwarded-for'] || '').split(',')[0].trim()) || req.socket.remoteAddress || '';
  if (bannedIPs.has(ws.remoteIP)) { ws.close(); return; }
  ...
  ws.on('message', (raw) => {
    ...
    if (msg.t === 'hostAction') {
      ...
      } else if (msg.action === 'ban' && msg.targetId) {
        const target = room.players.get(msg.targetId);
        if (target) {
          if (target.ws.remoteIP) bannedIPs.add(target.ws.remoteIP);
          if (target.ws.readyState === WebSocket.OPEN) {
            target.ws.send(JSON.stringify({ t: 'kicked', banned: true }));
            target.ws.close();
          }
        }
...
function tickRoom(room, now, dt) {
  ...
  for (const p of room.players.values()) {
    ...
    if (len > 0.05) {
      ...
      const boosted = p.speedBoostUntil && p.speedBoostUntil > now;
      const speed = TANK_SPEED * (boosted ? SPEED_BUFF_MULT : 1) * Math.min(len, 1);
    ...
    if (room.speedBuff && dist(p, room.speedBuff) <= room.speedBuff.r + TANK_RADIUS) {
      p.speedBoostUntil = now + SPEED_BUFF_DURATION;
      room.speedBuff = null;
    }
  }

  // --- бафф скорости: только командный бой, каждые 30 секунд ---
  if (room.mode === 'team' && now >= room.nextBuffSpawnAt) {
    spawnSpeedBuff(room);
    room.nextBuffSpawnAt = now + SPEED_BUFF_INTERVAL;
  }
...
function buildStateFor(room, viewer) {
  ...
  for (const p of room.players.values()) {
    ...
    playersOut.push({
      ...
      boosted: !!(p.speedBoostUntil && p.speedBoostUntil > Date.now())
    });
  }
  const bulletsOut = [];
